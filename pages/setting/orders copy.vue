                ...
                <ul>
                  <li v-for="product in props.row.products" :key="product.id">
                    ...
                    <span v-else :class="statusColor(product.status)">
                      {{ product.status }}
                      <q-icon v-if="product.status === '拒絕' && product.cancel_reason" name="help" size="xs" class="cursor-pointer text-body2">
...
const fetchOrders = async () => {
  try {
    ...
    Swal.fire({
      ...
      title: '失敗',
      text: errorMessage,
      backdrop: `https://media.tenor.com/sXhL2fgkn-IAAAAM/%E7%B5%B1%E7%A5%9E.gif`
    })
  }
}
